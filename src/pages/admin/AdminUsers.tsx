...
import { User, Mail, School, Trash2, Edit2, Search, X, AlertCircle, CheckCircle2, XCircle, Clock } from "lucide-react";
...
const AdminUsers = () => {
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      if (!user) return;
      const token = await user.getIdToken();
      await apiService.approveCandidate(id, token);
      toast.success("User approved and notified 📩");
      setUsers(users.map(u => u.id === id ? { ...u, isApproved: true, status: "approved" } : u));
    } catch (error) {
  ...
  const filteredUsers = users.filter(u => {
    ...
    const matchesStatus = filterStatus === "all" || (u.status || "pending") === filterStatus;
    return matchesSearch && matchesSemester && matchesYear && matchesDomain && matchesStatus;
  });
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 bg-secondary/20">
                  ...
                  <th className="p-4 text-sm font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                    ...
                    >
                      ...
                      <td className="p-4">
                        {user.status === "approved" || user.isApproved ? (
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-[11px] font-semibold border border-emerald-500/20">
                            <CheckCircle2 className="w-3 h-3" /> Approved
                          </div>
                        ) : user.status === "rejected" ? (
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-destructive/10 text-destructive text-[11px] font-semibold border border-destructive/20">
                            <XCircle className="w-3 h-3" /> Rejected
                          </div>
                        ) : (
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-500 text-[11px] font-semibold border border-amber-500/20">
