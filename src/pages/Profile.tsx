...
const Profile = () => {
  ...
  useEffect(() => {
    if (!loading && !errorState) {
      ...
      const validSections = ["Profile & Bio", "Academic Records", "Projects & Research", "Skills & Socials", "Certificates", "Account Security", "Resume Vault"];
  ...
  return (
    <div className="bg-ghost-white min-h-screen pt-28 pb-12 px-4 font-inter selection:bg-electric-blue/30 overflow-hidden">
            ...
            <section ref={sectionRefs["Profile & Bio"]} className="space-y-6">
              <div className="glass rounded-[2rem] border border-slate-200 p-8 shadow-sm space-y-8">
                  ...
                  <div className="grid md:grid-cols-12 gap-8">
                    <div className="md:col-span-4 flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50 relative">
                        <input 
                        ...
                        <div 
                          ...
                          className="w-32 h-32 rounded-full bg-slate-100 flex items-center justify-center mb-4 overflow-hidden relative border-4 border-white shadow-lg cursor-pointer group hover:scale-105 transition-all duration-300 ring-2 ring-slate-200 hover:ring-electric-blue"
                          title={formData.photoURL ? "Click to view full photo" : "Click to upload photo"}
                        >
                          {formData.photoURL ? (
                            <>
                              <img src={formData.photoURL} alt="Profile" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-opacity text-white backdrop-blur-[2px]">
                                <Eye className="w-6 h-6 drop-shadow-md" />
                                <span className="text-[9px] font-black uppercase tracking-wider mt-1 drop-shadow-md">View DP</span>
                              </div>
                            </>
                          ) : (
                            <div className="flex flex-col items-center">
                              <Upload className="w-8 h-8 text-slate-400 group-hover:text-electric-blue transition-colors" />
                        ...
                        <div className="flex items-center gap-2">
                          <label 
                            ...
                            className="text-[11px] font-black text-electric-blue hover:text-blue-700 uppercase tracking-wider cursor-pointer py-1.5 px-3 rounded-xl hover:bg-electric-blue/10 transition-colors border border-electric-blue/20"
                          >
                          ...
                          {formData.photoURL && (
                            <button
                              ...
                              className="text-[11px] font-black text-slate-600 hover:text-navy uppercase tracking-wider py-1.5 px-3 rounded-xl hover:bg-slate-200/60 transition-colors border border-slate-200 flex items-center gap-1.5"
                            >
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b border-slate-50">
                        <th className="py-4 text-[10px] font-black text-black uppercase">Current</th>
                        <th className="py-4 text-[10px] font-black text-black uppercase">Semester</th>
                        <th className="py-4 text-[10px] font-black text-black uppercase text-center">CGPA</th>
                        <th className="py-4 text-[10px] font-black text-black uppercase text-right">Marksheet Artifact</th>
                        <th className="py-4 text-[10px] font-black text-black uppercase text-right">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {formData.semesters.map((sem) => (
                        <tr key={sem.id} className="group">
                          ...
                          <td className="py-6 font-bold text-black">Semester 0{sem.id}</td>
                          <td className="py-6">
                            <input 
                              type="number" 
                              step="0.01"
                              className="w-20 mx-auto block px-3 py-2 rounded-xl bg-white border border-border text-center font-black text-black shadow-sm"
                              value={sem.cgpa}
                  ...
                  <div className="flex items-center justify-between border-b border-slate-100 pb-6">
                    ...
                    <div className="flex items-center gap-3">
                      {formData.resumeURL && (
                         <button 
                           type="button"
                           onClick={() => downloadOrOpenPdf(formData.resumeURL, `${formData.fullName || "Candidate"}_Resume.pdf`, true)}
                           className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-all cursor-pointer"
                         >
      ...
      <ProfilePhotoModal
        ...
        subtitle={formData.semester ? `Semester ${formData.semester}` : undefined}
      />
