...
import { 
  ...
  Instagram, BookOpen, Eye
} from "lucide-react";
import { toast } from "sonner";
import { downloadOrOpenPdf } from "@/lib/utils";
...
const Dashboard = () => {
  ...
  const handlePullResume = async (openInNewTab = false) => {
    if (!userData?.resumeURL) {
      toast.error("No resume found in vault");
      return;
    }
    const candidateName = userData?.fullName 
      ? userData.fullName.trim().replace(/[^a-zA-Z0-9_-]/g, "_") 
      : "Candidate";
    const toastId = toast.loading("Accessing resume from vault...");
    await downloadOrOpenPdf(userData.resumeURL, `${candidateName}_Resume.pdf`, openInNewTab);
  ...
  return (
    <div className="relative min-h-screen bg-ghost-white pt-24 pb-12 font-inter">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="mb-8 flex justify-between items-end">
            <div className="flex items-center gap-4">
              <div>
                <h1 className="text-[32px] font-bold text-text leading-none tracking-tight">SCHOLAR <span className="text-navy">INTEL</span></h1>
                <p className="text-[12px] font-medium text-muted-foreground uppercase tracking-widest mt-2">Real-time academic & professional overview</p>
              </div>
            </div>
            <button 
          ...
          <aside className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <div className="text-center space-y-6 relative z-10">
                <div 
                  className={`relative inline-block ${userData?.photoURL ? "cursor-pointer group" : ""}`}
                  onClick={() => {
                    if (userData?.photoURL) setIsPhotoModalOpen(true);
                  }}
                  title={userData?.photoURL ? "Click to view full profile photo" : undefined}
                >
                  <div className="w-32 h-32 rounded-[2rem] bg-deep-slate flex items-center justify-center text-white text-4xl font-black shadow-2xl mx-auto border-4 border-white overflow-hidden transition-all duration-300 group-hover:scale-105 group-hover:shadow-electric-blue/20 group-hover:border-navy-lt relative">
                    {userData?.photoURL ? (
                      <>
                        <img src={userData.photoURL} alt="Avatar" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all duration-200 flex flex-col items-center justify-center text-white backdrop-blur-[2px]">
                          <Eye className="w-6 h-6 text-white drop-shadow-md" />
                          <span className="text-[9px] font-black uppercase tracking-wider text-white drop-shadow-md mt-1">View DP</span>
                        </div>
                      </>
                    ) : (
                      userData?.fullName?.[0] || "?"
                  ...
                  <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl bg-emerald-500 border-4 border-white flex items-center justify-center shadow-lg pointer-events-none">
                ...
                <div className="pt-4 text-left px-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-[17px] font-semibold text-text uppercase tracking-[0.05em]">Tech Stack</h4>
                    <button 
                      onClick={() => navigate("/profile#Skills-&-Socials")}
                      className="text-[12px] font-medium text-muted hover:text-navy uppercase transition-colors"
                    >
                    ...
                    <>
                      <div className="flex flex-wrap gap-2">
                        {(showAllSkills ? skills : skills.slice(0, 5)).map((skill: string, idx: number) => (
                          <span key={idx} className="px-3 py-1.5 rounded-full bg-sky text-navy text-xs font-semibold border border-border shadow-sm">
                            {skill}
                          </span>
                        ))}
                      </div>
                      {skills.length > 5 && (
                        <button 
                          ...
                          className="mt-4 text-[12px] font-medium text-navy hover:text-navy-mid w-full text-center flex items-center justify-center gap-1 transition-colors"
                        >
          ...
          <main className="lg:col-span-2 space-y-10">
            ...
            <section className="space-y-5 mb-10">
              <div className="flex items-center justify-between px-4">
                <h3 className="text-[17px] font-semibold text-text uppercase tracking-[0.1em] flex items-center gap-3">
              ...
              <motion.div
                ...
                className="glass p-10 rounded-[2.5rem] border border-border shadow-lg relative overflow-hidden group"
              >
            ...
            <section className="space-y-5 mb-10">
              <div className="flex items-center justify-between px-4">
                <h3 className="text-[17px] font-semibold text-text uppercase tracking-[0.05em] flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {semesters.filter((s: any) => s.cgpa).map((sem: any, idx: number) => (
                  <motion.div 
                    ...
                    className="glass p-6 rounded-[2rem] border border-border hover:border-navy/30 transition-all flex flex-col md:flex-row items-center gap-6 group relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 w-32 h-32 bg-sky/50 rounded-bl-[100px] -z-10 group-hover:scale-110 transition-transform duration-700" />
                    
                    <div className="w-16 h-16 rounded-[1.5rem] bg-sky flex flex-col items-center justify-center shrink-0 border border-border">
                      <span className="text-[10px] font-bold text-black uppercase">Sem</span>
                      <span className="text-[24px] font-bold text-navy leading-none">{sem.id}</span>
                    </div>
                    
                    <div className="flex-1 text-center md:text-left">
                      <div className="flex flex-col md:flex-row md:items-baseline gap-2 mb-1">
                        <span className="text-sm font-bold text-black uppercase tracking-wider">GPA / Score</span>
                        <span className="text-[24px] font-extrabold text-black leading-none">{sem.cgpa || "N/A"}</span>
                      </div>
                      <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-3">
                        <motion.div 
                          ...
                          className="h-full bg-gradient-to-r from-[var(--navy)] to-[var(--navy-mid)]"
                        />
                      </div>
                    </div>
                  </motion.div>
            ...
            >
              ...
              <div className="relative space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    onClick={() => navigate("/profile#Resume-Vault")}
                    title="Upload or Update Resume"
      ...
      <ProfilePhotoModal
        ...
        subtitle={userData?.researchField || (userData?.semester ? `Semester ${userData.semester}` : undefined)}
      />
    </div>
  );
};
