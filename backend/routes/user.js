...
router.post("/upload/marksheet", verifyToken, upload.single("marksheet"), async (req, res) => {
  try {
    ...
    const originalName = req.file.originalname
      ? path.parse(req.file.originalname).name.replace(/[^a-zA-Z0-9_-]/g, "_")
      : "marksheet";
    const ext = path.extname(req.file.originalname || "") || ".pdf";
    const publicId = `${originalName}_${Date.now()}${ext}`;

    // Stream upload to Cloudinary preserving extension
...
router.put("/updateProfile", verifyToken, async (req, res) => {
  try {
    ...
    const updatedData = {
      ...
      scholarUrl: scholarUrl !== undefined ? scholarUrl : (existing.scholarUrl || ""),
      researchField: researchField !== undefined ? researchField : (existing.researchField || ""),
